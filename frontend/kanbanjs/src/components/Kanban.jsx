...
const KanbanС = (props) => {
  const { showInvisibleColumns, selectedCardUseState, draggingBoardUseState } =
    props;
      ...
      return Object.entries(colDict).map(([key, value]) => {
        ...
        return (
          <ColumnComponent
            ...
            status={k}
            title={value}
            cards={kanban.cards.filter((el) => el.status === k)}
          />
        );
      });
    }
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          flexDirection: "row",
          width: "100%",
          transform: draggingEnabled
            ? `translate(${translateBoard.x}px, ${translateBoard.y}px) scale(${scale})`
